...
'''
import cerebellum_10_250_ln as mice
import numpy as np
import random
import os
...
for n_mice in range(0, number_of_mice):
    ...
    w_inc_pfc_i = round(random.uniform(0.8, 1.4), 2) #w inc pfc to m1 spike
    w_inc_pfc_i_nsp=round(random.uniform(0.0000013, 0.0000020), 8) #w pfc to m1 no spike 13/15
    
    wf = round(random.uniform(7., 10.), 2) #initializzation w kernel
    ws = wf 
    weight_k_increment_i_f=round(random.uniform(0.000002, 0.00002), 8) #costant w increment weight Granular cells to PJ no spike
    ...
    weight_gain_i = round(random.uniform(7., 7.),2) #gain
    Ie_pfc_random = round(random.uniform(300.,365.),2)
    Ie_m1_random = round(random.uniform(300., 365.),2)
    
    w_n_nd_f_to_n_m1 = round(random.uniform(60, 60), 2)
    w_n_nd_s_to_n_pfc = w_n_nd_f_to_n_m1
